...
TUTOR_COURSE_MAP = {
    23: 1,  # Rosa Rivera (Lucy Rivera): Inicial 2 (ID 1)
    24: 2,  # Fátima Saavedra: 1° EGB (ID 2)
    13: 3,  # Lissette Ley: 2° EGB (ID 3)
    5: 4,   # Mercedes Carbo: 3° EGB (ID 4)
    18: 5,  # Gabriela Moreno: 4° EGB (ID 5)
    8: 6,   # Lupe Dominguez: 5° EGB (ID 6)
    31: 7,  # Anabel Falcones: 6° EGB (ID 7)
    9: 8,   # Lelis Fernandez: 7° EGB (ID 8)
    26: 9,  # Verónica Valencia: 8° EGB (ID 9)
    17: 10, # Diego Morán: 9° EGB (ID 10)
    19: 11, # Sonnia Muñoz: 10° EGB (ID 11)
    16: 12, # Deisy Mite: 1° BGU (ID 12)
    4: 13,  # Ángela Castillo: 2° BGU (ID 13)
    7: 14   # Israel Del Valle: 3° BGU (ID 14)
}
...
DOCENTE_DEFAULT_MATERIA = {
    ...
    16: 31, # Deisy Mite: Programación / ICT / Misa
...
def is_free_activity(text):
    ...
    for kw in ['PREPARACIÓN', 'PLANIFICACIÓN', 'PLANIFICACION', 'MATERIAL DIDÁCTICO', 'MATERIAL DIDACTICO', 
        ...
        if kw in t and not ('INGLÉS' in t or 'MATEMÁTICA' in t or 'LENGUA' in t or 'SOCIALES' in t or 'NATURALES' in t):
            return True
    return False
...
for f in sorted(glob.glob('HORARIOS DOCENTES/*.pdf')):
    ...
    print(f"ID {doc_id:2d} ({base:30s}): Clases: {len(clases):2d} | Sin Materia: {len(clases_sin_materia)} | Sin Curso: {len(clases_sin_curso)}")
