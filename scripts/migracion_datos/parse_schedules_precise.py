...
CURSO_MAP = {
    ...
    '1° BGU': 12, 'PRIMERO BGU': 12, '1ERO BGU': 12, '1RO BGU': 12, '1 BGU': 12, '1RO': 12, '1ERO': 12, 'PRIMERO': 12,
...
MATERIA_MAP = {
    ...
    'INGLÉS': 22, 'INGLES': 22,
    'INVESTIGACIÓN': 23, 'INVESTIGACION': 23, 'I.C T.': 23, 'ICT': 23,
    ...
    'ROBÓTICA': 31, 'ROBOTICA': 31, 'ROBÓTICA Y HABILIDADES COMPUTACIONALES': 31,
...
DOCENTE_DEFAULT_MATERIA = {
    ...
    16: 5,  # Deisy Mite: Catequesis
    17: 11, # Diego Morán: ECA
    24: 25, # Fátima Saavedra: Inicial 2 / Primaria
    18: 12, # Gabriela Moreno: Educación Física
    ...
    13: 22, # Lissette Ley: Inglés
    8: 24,  # Lupe Dominguez: Lengua y Literatura
    20: 7,  # Mayra Navarro: Primero EGB
    5: 11,  # Mercedes Carbo: ECA
    23: 24, # Rosa Rivera: Lengua y Literatura
...
def is_free_activity(text):
    t = text.upper()
    for kw in ['PREPARACIÓN', 'PLANIFICACIÓN', 'PLANIFICACION', 'MATERIAL DIDÁCTICO', 'MATERIAL DIDACTICO', 
               'RECUPERACIÓN', 'RECUPERACION', 'AMBIENTES', 'CALIFICACIÓN', 'CALIFICACION', 'INFORMES', 
               'ACOMPAÑAMIENTO', 'RECESO', 'SALIDA', 'LUNCH', 'ENTRADA', 'ACTUALIZACIÓN', 'REUNIÓN']:
        if kw in t:
            return True
    return False
...
def parse_teacher(pdf_path, docente_id):
    ...
    slots = []
    # Extract cells for each period and column
    for col_idx, (dia_name, x_min, x_max) in enumerate(col_bounds):
        ...
        for r_idx, row_info in enumerate(valid_time_rows):
            ...
            if r_idx == 0:
                ...
                y_top = (valid_time_rows[r_idx-1]['y'] + y_cur) / 2
                
            if r_idx == len(valid_time_rows) - 1:
                y_bottom = y_cur - 20
            else:
                y_bottom = (y_cur + valid_time_rows[r_idx+1]['y']) / 2

            cell_els = [e for e in col_elements if y_bottom <= e['y'] < y_top]
            cell_els.sort(key=lambda e: (-e['y'], e['x']))
            cell_text = clean_text(' '.join([e['text'] for e in cell_els]))

            # Check double period
            if not cell_text and r_idx > 0:
                prev_slot = next((s for s in slots if s['diaSemana'] == dia_name and s['franjaHorariaId'] == valid_time_rows[r_idx-1]['franja_id']), None)
                y_prev = valid_time_rows[r_idx-1]['y']
                mid_els = [e for e in col_elements if y_bottom <= e['y'] < y_prev + 10]
                if prev_slot and prev_slot['esClase'] and len(mid_els) > 0:
                    # Double period
                    slots.append({
                        'diaSemana': dia_name,
                        'franjaHorariaId': row_info['franja_id'],
...
for f in sorted(glob.glob('HORARIOS DOCENTES/*.pdf')):
    ...
    if doc_id in [1, 31]:
        print(f"Skipping {base} (ID {doc_id}) per user instruction (Alexandra Acosta / Anabel Falcones).")
        continue

    slots = parse_teacher(f, doc_id)
    clases = [s for s in slots if s['esClase']]
    libres = [s for s in slots if not s['esClase']]
    print(f"Docente ID {doc_id:2d} ({base:30s}): Total slots: {len(slots):2d} | Clases (Ocupado): {len(clases):2d} | Libres (Disponible): {len(libres):2d}")
